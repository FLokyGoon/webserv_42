...
/*   Updated: 2024/03/21 16:12:55 by fbardeau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
string WebServer::handleRequest(int clientSocket, const std::string& request) 
{
    ...
    if ((method == "GET" && get_2 == 0 && path == "/upload") || (method == "POST" && post_2 == 0 && path == "/upload") || (method == "DELETE" && del_2 == 0 && path == "/delete"))
    {
        cerr << "Method " << method << " not allowed" << endl;
        return sendError405();
    }
    ...
    {
        if (path == "/cgi-get")  
            return executeCGI(socketToServerConfigMap[clientSocket]._script3, request, clientSocket);
        else if(path == "/cgi_php")
        {
            string debug = executeCGI(socketToServerConfigMap[clientSocket]._script2, request, clientSocket);
            return sendTextResponse(clientSocket, debug);
        }
        else if (path == "/")
            ...
            return sendFileResponse(clientSocket, socketToServerConfigMap[clientSocket]._upload);
    ...
    else if (method == "POST") 
    {
        ...
        else if (path == "/cgi-post")
            return executeCGI("./html/cgi-bin/script_post.py", request, clientSocket);
        else if (path=="/login")
        {
    ...
    {
        if (path == "/delete")
            return deleteAllFilesInDirectory("./html/upload/");
        else
        {    
