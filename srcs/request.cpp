...
/*   Updated: 2024/03/21 15:51:29 by fbardeau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t WebServer::handle_outcoming_connection(size_t i)
{
    ...
    if (!_response.empty())
    {
        ...
        else
        {
            ...
            else if (_response.find("500 Internal Server Error") != string::npos)
            {
              cerr << "Socket Close 500 server error\n";
              close(fds[i].fd);
              clientSocketToPortMap.erase(fds[i].fd);
              fds.erase(fds.begin() + i);
              --i;
            ...
            {
                cerr << "Socket Close Not in Keepalive\n";
                close(fds[i].fd);
                clientSocketToPortMap.erase(fds[i].fd);
                fds.erase(fds.begin() + i);
                --i;
